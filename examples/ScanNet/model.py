...
# linear before identity 
class RefinementResidualBlock1(nn.Module):
    def __init__(self, dimension, a,b,leakiness=0):
        nn.Module.__init__(self)
        self.dimension = dimension
        self.a = a
        self.b = b
        self.linear=LinearSCN(a,b)
        self.BN1 = scn.BatchNormLeakyReLU(a,leakiness=leakiness)
        self.SCN1 = scn.SubmanifoldConvolution(dimension, b, b, 3, False)
        self.BN2 = scn.BatchNormLeakyReLU(b,leakiness=leakiness)
        self.SCN2 = scn.SubmanifoldConvolution(dimension, b, b, 3, False)
        # if a==b:
        self.direct = scn.Identity()
    ...
    def forward(self,x):
        y = self.linear(x)
        y1 = self.direct(y)
        y2 = self.SCN2(self.BN2(self.SCN1(self.BN1(y))))
        return scn.add_feature_planes([y1,y2])

# linear after identity  * 1~2% improvement on small model
class RefinementResidualBlock2(nn.Module):
    def __init__(self, dimension, a,b,leakiness=0):
        nn.Module.__init__(self)
        self.dimension = dimension
        self.a = a
    ...
    def forward(self,x):
        y1 = self.direct(x)
        y2 = self.SCN2(self.BN2(self.SCN1(self.linear(self.BN1(y)))))
        return scn.add_feature_planes([y1,y2])
...
class RepResidualBlock(nn.Module):
    def __init__(self,reps,dimension, a,b,leakiness=0):
        ...
        # self.res.append(RefinementResidualBlock1(dimension, a,b,leakiness=leakiness))
        self.res.append(RefinementResidualBlock2(dimension, a,b,leakiness=leakiness))
        for _ in range(reps-1):
            self.res.append(RefinementResidualBlock2(dimension, b,b,leakiness=leakiness))
    def forward(self, x):
