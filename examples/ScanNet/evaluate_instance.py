...
def region_based_cross_modal_meanshift_merging(exsiting_offsets, pred_embedding,pred_semantic, pred_displacements, xyz, pred_bw, regions, occupancy_size):
    # First merging, no occupancy, similairty 0.6
    # Second merging, with occupancy, similarity 0.5, merge()
    instance_count = 0
    ...
    # repeat until no similarity is above 0.6
    similar_pair = torch.argmax(similarity_map)
    similarity_value = similarity_map[similar_pair / center_num,similar_pair % center_num].clone()
    ...
    while instance_count <  expected_instance_num and background_mask.sum(0) > minimum_instance_size * 0.5:
        ...
        for k in range(10):
            ...
            if(mean_shift_vector.norm(dim = 0) < 1e-5):
                break
#        print(instance_flag.sum().item())
        # print('center_semantic[instance_flag].shape:')
        # print(center_semantic[instance_flag].shape)
        # print('stats.mode(center_semantic[instance_flag].cpu().numpy())[0]')
        # print(stats.mode(center_semantic[instance_flag].cpu().numpy())[0])
        if instance_flag.sum().item() == 0:
...
def joint_semantic_instance_segmentation(pred_semantic_probability, true_semantic,exsiting_offsets, pred_embedding,pred_semantic, pred_displacements, xyz, pred_bw, regions):
    ...
    # Build the initial graph from preclustering results.
    [region_index, region_mask] = np.unique(regions, False, True)
    region_mask = torch.from_numpy(region_mask).cuda()
    center_num = region_index.shape[0]
    # Compute average semantic prediction scores for each node
    ...
    # Get max offset for each node (Not used??)
    # pose_device = torch.tensor(xyz).cuda()
    # region_centers = scatter_mean(pose_device,region_mask,dim = 0)
    # region_range = torch.norm(pose_device - torch.gather(region_centers, dim = 0, index = region_mask.view(-1,1).expand(-1,3)),dim=1)
    # region_range,max_index_value = scatter_max(region_range, region_mask, dim = 0)
    ...
    # Compure average feature embedding and predicted center
    similarity_map = torch.zeros(2,center_num, center_num)
    pred_center_embedding_device = gaussian_weighted_region_mean(pred_embedding_device, regions)  # feature embedding
    pose_center_embedding_device = gaussian_weighted_region_mean(pose_embedding_device, regions)  # spatial embedding
    """
    clustering_centers = torch.zeros(center_num, feature_dim) 
    for i in range(center_num):
        new_center = pred_center_embedding_device[i,:]
...
#    pdb.set_trace()
    ...
    # Concat feature and spatial
    pred_embedding_device = torch.cat([pred_center_embedding_device, pose_center_embedding_device], dim = 1)
    pred_instance_points = torch.zeros((point_num,), dtype=torch.int32).cuda()

    pred_instance = torch.zeros((center_num,), dtype=torch.int32).cuda()
    background_mask = torch.tensor(np.ones((center_num,), dtype=np.int)).cuda() > 0
    # Acerage covariance term
    pred_bw_device = gaussian_weighted_region_mean(torch.tensor(pred_bw).cuda(), regions)
    # Node semantic label by taking the mode
    center_semantic = torch.zeros((center_num,))
    for i in range(center_num):
        center_semantic[i] = int(stats.mode(pred_semantic[regions == region_index[i]])[0])
    # copy of pred_offsets
    exsiting_offsets_device = torch.tensor(exsiting_offsets).cuda() 
    # Largest offset of nodes
    region_point_num = scatter_add(torch.ones([point_num]).cuda(), region_mask, dim = 0)
    exsiting_offsets_device, max_index_value = scatter_max(exsiting_offsets_device, region_mask, dim = 0)
    ...
    while instance_count <  expected_instance_num and background_mask.sum(0) > minimum_instance_size * 0.5:
        ...
        background_mask[keypoint] = 0                # a flag for non-instance nodes
        exsiting_offsets_device[keypoint] = 0        # clear offset
        instance_count += 1
...
def joint_instance_semantic_prediction(files, args):
    ...
    gt_pts = []
    joint_pts = []
    train_writer = SummaryWriter(comment=args.taskname+"joint_instance_segmentation")
    ...
    for file_index,file in enumerate(files):
        ...
        # print("[DEBUG] gt:", gt, true_semantic)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'scannet':
        ...
        if args.evaluate:
            candidate_path = './datasets/scannet_data/instance/test/' + args.taskname + '/*.npz'
        else:
            candidate_path = './datasets/scannet_data/instance/val/'+ args.taskname + '/*.npz'
            # candidate_path = './datasets/scannet_data/instance/val/' + args.taskname + '/*scene0208_00_vh_clean_2_instance.npz'

        if args.simple_train:
            candidate_path = './datasets/scannet_data/instance/simple/' + args.taskname + '/*.npz'
        class_num = 20
    ...
    # Get refined semantic predictions
    ...
    for file_index,file in enumerate(files):
        ...
        results = np.load(file)
        pred_semantic = results['pred_semantic']                            # Nx1, semantic label
        pred_embedding = results['pred_embedding']                          # NxD
        true_semantic = results['true_semantic']                            # Nx1, semantic gt
        true_instance = results['true_instance']                            # Nx1, instance gt
        true_offsets = results['true_offsets']                              # Nx1, offset to center
        pred_offsets = results['pred_offsets']                              # Nx1
        true_displacements = results['true_displacements']                  # Nx3, vector from obj center to points
        pred_displacements = results['pred_displacements']                  # Nx3
        pred_bw = results['pred_bw']                                        # Nx2, covariance term
        regions = results['regions']                                        # Nx1, preclustering index
        pred_occupancy_size = results['occupancy']                          # Nx1, predicted occupancy
        true_occupancy_size = results['true_occupancy']                     # Nx1
        pred_semantic_probability = results['pred_semantic_probability']    # NxC, averaged semantic score 
...
#        open3d.visualization.draw_geometries([pcd_pred_instance, pcd_gt, pcd_gt_hot, pcd_ori])
    ...
    os.system('python /media/hdd/zhengtian/Occuseg/codebase/MASC/scripts/evaluate_semantic_instance.py')
